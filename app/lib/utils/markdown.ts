...
export function parseMarkdown(text: string): string {
  ...
  let result = escape(text)
    ...
    // Images
    .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<div class="relative flex items-center justify-center min-h-[200px] w-full max-w-sm rounded-lg bg-surface-2 overflow-hidden my-3 shadow-lg group"><div class="absolute inset-0 flex items-center justify-center image-loader"><span class="h-6 w-6 rounded-full border-2 border-accent border-t-transparent animate-spin"></span></div><img src="$2" alt="$1" class="generated-image z-10 w-full h-auto cursor-pointer" onload="this.previousSibling.style.display=\'none\'; this.style.opacity=1" style="opacity: 0; transition: opacity 0.5s" /><button type="button" class="watermark-download-btn absolute top-2 right-2 p-2 bg-black/60 backdrop-blur-md text-white rounded-lg opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-20 hover:bg-black/80 flex items-center gap-1.5 text-xs font-medium" title="Download Image" data-url="$2"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg></button></div>')
    // Images with error handling, automatic retry, and download button
    .replace(/\[IMAGE:\s*([^\]]+)\]/gi, '<div class="relative flex items-center justify-center min-h-[200px] w-full max-w-sm rounded-lg bg-surface-2 overflow-hidden my-3 shadow-lg group"><div class="absolute inset-0 flex items-center justify-center image-loader"><span class="h-6 w-6 rounded-full border-2 border-accent border-t-transparent animate-spin"></span></div><img src="$1" alt="Generated Image" class="generated-image z-10 w-full h-auto cursor-pointer" onload="this.previousSibling.style.display=\'none\'; this.style.opacity=1" onerror="if(!this.getAttribute(\'data-retried\')){this.setAttribute(\'data-retried\',\'1\');var img=this;setTimeout(function(){img.src=img.src.split(\'&retry=\')[0]+\'&retry=\'+Date.now();},3000);}else{this.previousSibling.innerHTML=\'<span class=\\\'text-xs text-red-400\\\'>Image error (Server overloaded)</span>\';this.style.display=\'none\';}" style="opacity: 0; transition: opacity 0.5s" /><button type="button" class="watermark-download-btn absolute top-2 right-2 p-2 bg-black/60 backdrop-blur-md text-white rounded-lg opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-20 hover:bg-black/80 flex items-center gap-1.5 text-xs font-medium" title="Download Image" data-url="$1"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg></button></div>')
    // Videos with error handling
    .replace(/\[VIDEO:\s*([^\]]+)\]/gi, '<div class="relative flex items-center justify-center min-h-[200px] w-full max-w-sm rounded-lg bg-black overflow-hidden my-3 group shadow-xl"><div class="absolute inset-0 flex items-center justify-center image-loader" id="vloader-${Date.now()}"><span class="h-6 w-6 rounded-full border-2 border-accent border-t-transparent animate-spin"></span></div><video src="$1" class="generated-video z-10 w-full h-auto" controls autoplay loop playsinline onloadeddata="this.previousSibling.style.display=\'none\'; this.style.opacity=1" onerror="this.previousSibling.innerHTML=\'<span class=\\\'text-xs text-red-400\\\'>Video unavailable</span>\'; this.style.display=\'none\'" style="opacity: 0; transition: opacity 0.5s"></video><div class="absolute bottom-2 right-2 px-2 py-0.5 bg-black/60 backdrop-blur-md text-[10px] font-bold text-white/90 rounded border border-white/10 z-20 pointer-events-none tracking-widest uppercase transition-opacity">✨ ORION AI</div></div>')
