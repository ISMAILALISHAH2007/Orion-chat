...
export function parseMarkdown(text: string): string {
  ...
  let result = escape(text)
    ...
    // Images
    .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<div class="relative flex items-center justify-center min-h-[200px] w-full max-w-sm rounded-lg bg-surface-2 overflow-hidden my-3 shadow-lg group"><div class="absolute inset-0 flex items-center justify-center image-loader"><span class="h-6 w-6 rounded-full border-2 border-accent border-t-transparent animate-spin"></span></div><img src="$2" alt="$1" class="generated-image z-10 w-full h-auto cursor-pointer" onload="this.previousSibling.style.display=\'none\'; this.style.opacity=1" style="opacity: 0; transition: opacity 0.5s" /><a href="$2" download="$1" target="_blank" rel="noopener noreferrer" class="absolute top-2 right-2 p-2 bg-black/60 backdrop-blur-md text-white rounded-lg opacity-0 group-hover:opacity-100 transition-opacity z-20 hover:bg-black/80 flex items-center gap-1.5 text-xs font-medium" title="Download Image"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg></a></div>')
    // Images with error handling and download button
    .replace(/\[IMAGE:\s*([^\]]+)\]/gi, '<div class="relative flex items-center justify-center min-h-[200px] w-full max-w-sm rounded-lg bg-surface-2 overflow-hidden my-3 shadow-lg group"><div class="absolute inset-0 flex items-center justify-center image-loader"><span class="h-6 w-6 rounded-full border-2 border-accent border-t-transparent animate-spin"></span></div><img src="$1" alt="Generated Image" class="generated-image z-10 w-full h-auto cursor-pointer" onload="this.previousSibling.style.display=\'none\'; this.style.opacity=1" onerror="this.previousSibling.innerHTML=\'<span class=\\\'text-xs text-red-400\\\'>Image unavailable</span>\'; this.style.display=\'none\'" style="opacity: 0; transition: opacity 0.5s" /><a href="$1" download="ultron_image.jpg" target="_blank" rel="noopener noreferrer" class="absolute top-2 right-2 p-2 bg-black/60 backdrop-blur-md text-white rounded-lg opacity-0 group-hover:opacity-100 transition-opacity z-20 hover:bg-black/80 flex items-center gap-1.5 text-xs font-medium" title="Download Image"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg></a></div>')
